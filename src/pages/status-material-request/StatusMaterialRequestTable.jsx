...
export default function StatusMaterialRequestTable() {
  ...
  const fetchStatuses = async () => {
    setLoading(true);
    try {
      const res = await api.get("/master-status-mr");
      setStatuses(res.data || []);
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Failed to load status material requests", "error");
      setStatuses([]);
    } finally {
      setLoading(false);
    }
  ...
  const saveForm = async () => {
    try {
      if (formData.id) {
        await api.put(`/master-status-mr/${formData.id}`, formData);
        Swal.fire("Success", "Status updated successfully", "success");
      } else {
        await api.post("/master-status-mr", formData);
  ...
  const deleteStatus = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await api.delete(`/master-status-mr/${id}`);
