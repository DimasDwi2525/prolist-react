...
export default function TypePackingListTable() {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const res = await api.get("/master-type-packing-lists");
      setTypes(res.data || []);
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Failed to load type packing lists", "error");
      setTypes([]);
    } finally {
      setLoading(false);
    }
  ...
  const saveForm = async () => {
    try {
      if (formData.id) {
        await api.put(`/master-type-packing-lists/${formData.id}`, formData);
        Swal.fire("Success", "Type updated successfully", "success");
      } else {
        await api.post("/master-type-packing-lists", formData);
  ...
  const deleteType = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await api.delete(`/master-type-packing-lists/${id}`);
  ...
  return (
    <Box sx={{ position: "relative" }}>
      ...
      >
        ...
        >
          {formData.id ? "Edit Type" : "Create Type"}
        </DialogTitle>
